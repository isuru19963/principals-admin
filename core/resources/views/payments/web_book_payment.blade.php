<?php
 $appData = session()->get('bookbuy_data');
// unique_order_id|total_amount
$odre_pay = $appData["amount"];
$odre_id = $appData["order_id"];
...
$custom_fields = base64_encode($odre_id.'|web|book|cus_4');
...
<html lang="en">
  ...
  <body>
      <form action="<?php echo $url; ?>" method="POST">
			...
			<input type="hidden" name="first_name" value="<?php echo $appData['first_name'];?>">
			<input type="hidden" name="last_name" value="<?php echo $appData['first_name'];?>">
			<input type="hidden" name="email" value="<?php echo $appData['email'];?>">
			<input type="hidden" name="contact_number" value="<?php echo$appData['contact_no'];?>">
			<input type="hidden" name="address_line_one" value="<?php echo $appData['address'];?>">
			...
			<input  name="payment" value="<?php echo $payment; ?>"  hidden>
