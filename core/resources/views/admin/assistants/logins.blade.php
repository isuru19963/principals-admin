                        ...
                        <table class="table table--light style--two">
                            ...
                            @forelse($login_logs as $log)
                                <tr>
                                    ...
                                    <td data-label="@lang('Username')"><a href="{{ route('admin.assistants.detail', $log->assistant_id)}}"> {{ ($log->assistant) ? $log->assistant->username : '' }}</a></td>
                                    <td data-label="@lang('IP')">
                                        <a href="{{route('admin.assistants.login.ipHistory',[$log->assistant_ip])}}">
                                            {{ $log->assistant_ip }}
                                        </a>
                                    </td>
...
@push('breadcrumb-plugins')
    @if(request()->routeIs('admin.assistants.login.history.single'))
    <form action="{{ route('admin.assistants.login.history') }}" method="GET" class="form-inline float-sm-right bg--white">
