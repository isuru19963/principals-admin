                        ...
                        <table class="table table--light style--two">
                            ...
                            @forelse($login_logs as $log)
                                <tr>
                                    ...
                                    <td data-label="@lang('Username')"><a href="{{ route('admin.staff.detail', $log->staff_id)}}"> {{ ($log->staff) ? $log->staff->username : '' }}</a></td>
                                    <td data-label="@lang('IP')">
                                        <a href="{{route('admin.staff.login.ipHistory',[$log->staff_ip])}}">
                                            {{ $log->staff_ip }}
                                        </a>
                                    </td>
...
@push('breadcrumb-plugins')
    @if(request()->routeIs('admin.staff.login.history'))
    <form action="{{ route('admin.staff.login.history') }}" method="GET" class="form-inline float-sm-right bg--white">
