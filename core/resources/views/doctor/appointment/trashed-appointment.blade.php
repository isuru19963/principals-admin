        ...
        <div class="col-lg-12">
            <div class="card">
                    ...
                    <div class="table-responsive table-responsive--sm">
                        <table class="default-data-table table">
                            ...
                            <tr>
                                ...
                                @if ($item->d_staff)
                                    <td data-label="@lang('Removed By')">{{ $item->deleteRelationStaff->name }} <span class="text--small badge font-weight-normal badge--danger">@lang('Staff')</span></td>
                                @elseif($item->d_assistant)
                                    <td data-label="@lang('Removed By')">{{ $item->deleteRelationAssistant->name }} <span class="text--small badge font-weight-normal badge--danger">@lang('Assistant')</span></td>
                                @elseif($item->d_doctor)
                                    <td data-label="@lang('Removed By')">{{ $item->deleteRelationDoctor->name }} <span class="text--small badge font-weight-normal badge--danger">@lang('Doctor')</span></td>
                                @elseif($item->d_admin)
                                    <td data-label="@lang('Removed By')">{{ $item->deleteRelationAdmin->name }} <span class="text--small badge font-weight-normal badge--danger">@lang('Admin')</span></td>
                                @else
                ...
                <div class="card-footer py-4">
                    {{ $appointments->links('assistant.partials.paginate') }}
                </div>
            </div><!-- card end -->
        </div>
    </div>
